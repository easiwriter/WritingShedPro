...
- `POST /api/sync/production/v1/identity/check` -> checks an already-registered user/device/project entitlement against the production schema. Missing user/device identity fields are rejected before DB work. This route requires `Authorization: Bearer <SYNC_PRODUCTION_TOKEN>`, performs no writes, and always reports `writesEnabled: false` in the current foundation phase.
- `POST /api/sync/production/v1/identity/enrollment/preflight` -> checks whether supplied user/device enrollment evidence is ready for a future identity writer. This route requires `Authorization: Bearer <SYNC_PRODUCTION_TOKEN>`, performs no writes, creates no users/devices/entitlements, always reports `readyToEnrollUser: false`, reports missing consent or backup/export evidence as blockers, reports unregistered and revoked user/device state without auto-creating records, and returns `production_identity_enrollment_not_implemented` until a disabled enrollment writer is explicitly implemented and tested.
- `POST /api/sync/production/v1/entitlements/preflight` -> checks whether an existing user/device/project relationship is ready for a future entitlement writer. This route requires `Authorization: Bearer <SYNC_PRODUCTION_TOKEN>`, performs no writes, creates or updates no project entitlements, always reports `readyToGrantProjectEntitlement: false`, reports unregistered identities, missing consent, missing projects, and revoked entitlements as blockers, and returns `production_project_entitlement_writer_not_implemented` until a disabled entitlement writer is explicitly implemented and tested.
- `POST /api/sync/production/v1/environment/preflight` -> reports production environment and rollout flag state before migration, apply, or write enablement can be considered. This route requires `Authorization: Bearer <SYNC_PRODUCTION_TOKEN>`, performs no writes, enables no rollout flags, reports missing environment setup, missing rollout flags, already-enabled writes, and active kill switches as blockers, always reports `readyToEnableProductionWrites: false`, and returns `production_environment_writes_not_enabled` until release gates pass.
- `POST /api/sync/production/v1/migration/preflight` -> evaluates supplied CloudKit project inventory, consent, backup/export, user/device/project entitlement, and source-state signals before a migration run can be planned. Missing project id is rejected before DB work. This route performs no writes, does not apply operations, reports missing consent, missing backup/export, wrong source system, and non-ready CloudKit state as blockers, and always reports `readyToApplyMigration: false` in the current foundation phase.
- `POST /api/sync/production/v1/assets/preflight` -> validates a supplied asset manifest, production blob binding, and user/device/project entitlement before asset transfer can be attempted. This route rejects missing asset identity fields, invalid byte counts, duplicate asset ids, and duplicate proposed R2 keys before DB/R2 work, performs no R2 upload, no D1 write, no operation apply, and always reports `readyToAdvanceCursor: false` until a later verified upload/checksum path exists.
- `POST /api/sync/production/v1/apply/preflight` -> validates an operation-window shape against existing identity, entitlement, project, and device cursor rows before production apply can be attempted. Sequence gaps and missing payload hashes are rejected before DB work. It reports unresolved dependencies and pending assets as blockers, performs no D1 write, does not mutate app SwiftData, and always reports `readyToApply: false` until a flagged SwiftData applier exists and commits before cursor advancement.
- `POST /api/sync/production/v1/orchestrator/eligibility` -> evaluates whether a launch, foreground, network-recovery, silent-push, background-refresh, or manual trigger is allowed for an existing user/device/project/cursor state. Unsupported triggers are rejected before DB work. It reports offline state, recovery-without-prior-failure, and background-refresh state mismatches as blockers. This route performs no writes, schedules no lifecycle work, does not apply operations, and always reports `eligibleToRun: false` until the production applier and rollout gates exist.
- `POST /api/sync/production/v1/release/readiness` -> evaluates supplied release evidence plus production environment and rollout flag rows before production writes can be considered. Missing evidence payloads are rejected before DB work; missing production environment setup, already-enabled writes, missing rollout flags, missing release evidence items, and active rollout kill switches are release blockers. This route performs no writes, enables no flags, and always reports `readyToRelease: false` until a separate audited enable path exists.
