...
Added 2026-07-08, self-validated: `POST /api/sync/production/v1/users/summary` added as a read-only user-counting foundation route. It requires `SYNC_PRODUCTION_TOKEN`, reads aggregate counts from `sync_users` and `sync_devices`, returns total/active/consented/revoked user and device counts, exposes no document content, project names, attachment payloads, or secrets, performs no writes, and keeps `writesEnabled` plus `appMutationEnabled` false.

Updated 2026-07-08, self-validated: `sync-production-foundation-contract.json` now marks `/users/summary` as `aggregateOnly`. The validator checks that the route uses aggregate counts from `sync_users` and `sync_devices` only, and fails if it starts reading project, operation, asset, tombstone, migration, audit, support, entitlement, subject, account, project-name, R2 key, or payload detail.

Updated 2026-07-08, self-validated: aggregate-only production route validation is now manifest-driven. Aggregate routes must declare their handler, allowed read tables, and forbidden detail fields; the validator extracts the handler body, verifies `COUNT(*)`, enforces the allowed `FROM sync_*` table set, and blocks detail-field reads.

Updated 2026-07-08, self-validated: added local no-Cloudflare production foundation smoke checks. `npm run sync:prod:smoke` imports the Worker with mocked bindings and exercises `/users/summary` unauthorized, missing DB, aggregate success response, no-store cache headers, and no D1 mutation calls. `sync:prod:validate:all` now runs the smoke check before the in-memory SQLite schema parse.

Updated 2026-07-08, self-validated: expanded the local smoke check to cover public production `/health`. It verifies health works without auth/bindings, reports binding presence when mocked bindings are supplied, keeps `writesEnabled: false` and `appMutationEnabled: false`, and returns `Cache-Control: no-store`.

Updated 2026-07-08, self-validated: expanded `sync:prod:smoke` routing coverage from the production contract manifest. The smoke check now verifies unknown production routes return `404`, `/health` rejects non-GET methods, and every protected production route rejects non-POST methods with `405` and missing auth with `401`, all with `Cache-Control: no-store`.

Updated 2026-07-08, self-validated: expanded protected-route smoke coverage to invalid JSON. Every protected production route is now exercised with valid auth, configured mock bindings, and malformed JSON; the mock DB throws if queried, so the check verifies the shared JSON parser returns `400` before route-specific DB work begins, with `Cache-Control: no-store`.

Updated 2026-07-08, self-validated: added `/assets/preflight` missing-blob smoke coverage. The mock DB returns an otherwise-ready user/device/project/entitlement and the request omits `SYNC_PRODUCTION_BLOBS`; the route must return `readyToTransferAssets: false`, `readyToAdvanceCursor: false`, `writesEnabled: false`, `appMutationEnabled: false`, blocker `production_blob_storage_not_configured`, normalized manifest details, and no-store headers without D1 mutations or R2 upload attempts.

Updated 2026-07-08, self-validated: added oversized asset manifest behavior coverage. `sync:prod:smoke` now submits 1001 asset manifest items to `/assets/preflight` with valid auth and configured mock bindings; a no-query DB mock proves the request is rejected with `400 { error: "assets exceeds 1000" }` and `Cache-Control: no-store` before any D1 or R2 work can begin.

Updated 2026-07-08, self-validated: added oversized apply-window behavior coverage. `sync:prod:smoke` now submits an operation window with 501 operations to `/apply/preflight` with valid auth and configured mock bindings; a no-query DB mock proves the request is rejected with `400 { error: "operationWindow.operationCount exceeds 500" }` and `Cache-Control: no-store` before any D1 query, app apply, or cursor work can begin.

Updated 2026-07-08, self-validated: added otherwise-ready `/apply/preflight` behavior coverage. The mock DB returns active user/device, writable project, valid entitlement, idle cursor, and matching operation window; the route must still return `readyToApply: false`, `readyToAdvanceCursor: false`, `writesEnabled: false`, `appMutationEnabled: false`, blocker `production_swiftdata_applier_not_implemented`, and current cursor/project sequence details with no D1 mutation or cursor advancement.

Updated 2026-07-08, self-validated: added otherwise-ready `/orchestrator/eligibility` behavior coverage. The mock DB returns active user/device, writable project, valid entitlement, idle cursor, network available, and a manual foreground trigger; the route must still return `eligibleToRun: false`, `eligibleToApply: false`, `eligibleToAdvanceCursor: false`, `writesEnabled: false`, `appMutationEnabled: false`, blockers `production_orchestrator_not_wired` and `production_swiftdata_applier_not_implemented`, and cursor/project sequence details with no lifecycle scheduling, D1 mutation, app apply, or cursor advancement.

Updated 2026-07-08, self-validated: added otherwise-ready `/release/readiness` behavior coverage. The mock DB returns one production environment with writes disabled plus rollout flags, and the request supplies all required release evidence as true; the route must still return `readyToRelease: false`, `readyToEnableProductionWrites: false`, `writesEnabled: false`, `appMutationEnabled: false`, blocker `production_release_enable_endpoint_not_implemented`, and environment/rollout summaries with no D1 mutation, rollout enablement, or app mutation.

Updated 2026-07-08, self-validated: added otherwise-authorized `/identity/check` behavior coverage. The mock DB returns active user/device and a non-revoked project entitlement with `can_write = 1`; the route must still return `authorized: true`, `canRead: true`, `canWrite: false`, `writesEnabled: false`, empty reasons, and no-store headers with no D1 mutation or write enablement.

Updated 2026-07-08, self-validated: added plan-only `/migration/preflight` behavior coverage. The mock DB returns active user/device, project with writes disabled, valid entitlement, acknowledged consent, verified backup/export, quiescent CloudKit state, and non-empty inventory; the route may return `readyToPlanMigration: true` and `nextStep: create_disabled_migration_run`, but must still return `readyToApplyMigration: false`, `writesEnabled: false`, `appMutationEnabled: false`, normalized inventory and batch estimates, and no-store headers with no D1 mutation, migration-run creation, R2 upload, SwiftData mutation, or apply path.

Updated 2026-07-08, self-validated: added route-specific smoke coverage drift protection. `smoke-sync-production-foundation.mjs` now keeps an explicit `routeSpecificSmokeCoverage` set and asserts it exactly matches every route declared in `sync-production-foundation-contract.json`; adding a production route now fails `sync:prod:smoke` until deliberate route-specific smoke coverage is added.

Updated 2026-07-08, self-validated: added transfer-ready `/assets/preflight` behavior coverage. The mock DB returns otherwise-ready user/device/project/entitlement and `SYNC_PRODUCTION_BLOBS` is supplied as a mock that throws on upload/delete/multipart calls; the route may return `readyToTransferAssets: true` and normalized transfer metadata, but must still return `readyToAdvanceCursor: false`, `writesEnabled: false`, `appMutationEnabled: false`, no blockers, and no-store headers without any R2 upload, D1 mutation, SwiftData mutation, or cursor advancement.

Updated 2026-07-08, self-validated: added protected-route auth misconfiguration coverage. `sync:prod:smoke` now sends a Bearer token to every protected production route while omitting `SYNC_PRODUCTION_TOKEN`; each route must return `401` with `Cache-Control: no-store` before DB configuration, JSON parsing, or route-specific work.

Updated 2026-07-08, self-validated: added protected-route production DB misconfiguration coverage. `sync:prod:smoke` now sends valid auth plus malformed JSON to every protected production route while omitting `SYNC_PRODUCTION_DB`; each route must return `503` with `Cache-Control: no-store` before JSON parsing or route-specific work.
...
Added 2026-07-08, self-validated: first post-POC identity implementation boundary added as `POST /api/sync/production/v1/identity/enrollment/preflight`. The route is authenticated and production-DB-backed, but remains read-only: it can report existing user/device state, consent/backup evidence, and blockers, but it creates no users, devices, projects, or entitlements, keeps `readyToEnrollUser: false`, and returns `production_identity_enrollment_not_implemented` until a disabled identity writer is explicitly implemented and tested.

Updated 2026-07-08, self-validated: identity enrollment preflight smoke now covers missing consent and backup/export evidence. The route must return `consent_not_acknowledged` and `backup_export_not_verified` blockers while keeping enrollment/device registration disabled, preserving no-store caching, and limiting DB work to read-only user/device checks.

Updated 2026-07-08, self-validated: identity enrollment preflight smoke now covers the unregistered identity path. When the supplied user/device is missing, the route reports `userAlreadyRegistered: false` and `deviceAlreadyRegistered: false` without auto-creating records, keeps enrollment/device/entitlement creation disabled, and limits DB work to the read-only user lookup.

Updated 2026-07-08, self-validated: identity enrollment preflight smoke now covers revoked user/device state. The route returns `user_revoked_or_deleted` and `device_revoked` blockers, keeps enrollment/device/entitlement creation disabled, preserves no-store caching, and limits DB work to read-only user/device checks.

Added 2026-07-08, self-validated: first project entitlement implementation boundary added as `POST /api/sync/production/v1/entitlements/preflight`. The route is authenticated and production-DB-backed, but remains read-only: it reports existing user/device/project/entitlement state, keeps `readyToGrantProjectEntitlement: false` and `readyToUpdateProjectEntitlement: false`, returns `production_project_entitlement_writer_not_implemented`, creates or updates no entitlements, and limits DB work to identity, device, project, and entitlement reads.

Updated 2026-07-08, self-validated: project entitlement preflight smoke now covers missing project and revoked entitlement state. The route returns `project_not_registered` or `project_entitlement_revoked` blockers while keeping grant/update readiness disabled, preserving no-store caching, and limiting DB work to read-only identity/device/project/entitlement checks.

Updated 2026-07-08, self-validated: project entitlement preflight smoke now covers the unregistered identity path. When the supplied user is missing, the route reports missing user/device/project/entitlement state without auto-creating records, keeps grant/update readiness disabled, and limits DB work to the read-only user lookup.
